...
function AuditLog() {
    ...
    /* logEvent( actor, action, label, object, status, description )
     *
     ...
     * examples:
     *
     *   actor - time - origin - action - label - object - status - description
     ...
     *
     */
    this.logEvent = function( actor, origin, action, label, object, status, description ) {
        if(!actor && this._userId) actor = this._userId; // set the actor if it wasn't provided.
        
        var eventPackage = this.buildEventPackage(actor, new Date(), origin, action, label, object, status, description);
    ...
    /* buildEventPackage( actor, date, origin, action, label, object, status, description )
     *
     * returns an object containing event options for transmission to transports
     */
    this.buildEventPackage = function(actor, date, origin, action, label, object, status, description) {
        var defaults = { actor: '', date: new Date(), origin: '', action: '',
            label: '', object: '', status: '', description: '' };
